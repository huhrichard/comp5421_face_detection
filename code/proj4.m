...
pos_extract_Time = toc

tic;
fprintf('Getting negative features...\n')
...
lambdaList = 10.^(-5:0.5:-3);
paraList = [];
bestW = 0;
bestb = 0;
largestGap = 0;
bestLambda = 1;
features_posT = features_pos';
features_negT = features_neg';
fprintf('Training SVM...\n')
tic;
for lambda = lambdaList
	[w, b, INFO, SCORES] = vl_svmtrain(X, Y, lambda);

	confidences = [features_posT; features_negT]*w + b;
	label_vector = [ones(size(features_posT,1),1); -1*ones(size(features_negT,1),1)];
	...
	gap = mean(face_confs)-mean(non_face_confs);
	if gap > largestGap
		largestGap = gap;
		bestW = w;
		bestb = b;
...
non_face_confs = confidences( label_vector < 0);
face_confs     = confidences( label_vector > 0);
threshold = (mean(non_face_confs)+mean(face_confs))/2
% threshold = -1;
figure(2); 
...
% extra_test_path = fullfile(data_path, 'train_non_face_scenes');
